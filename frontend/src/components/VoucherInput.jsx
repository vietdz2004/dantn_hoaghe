...
const VoucherInput = ({ onVoucherApplied, onVoucherRemoved, orderTotal = 0, appliedVoucher, userId, productIds }) => {
  ...
  const handleApplyVoucher = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // setAppliedVoucher(voucherData); // This line is removed as per the new_code
  ...
  const handleRemoveVoucher = () => {
    // setAppliedVoucher(null); // This line is removed as per the new_code
    setSuccess('');
    setError('');
      ...
      {!voucher ? (
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
            <Chip 
              label={voucher.voucher.maVoucher} 
              color="success" 
              variant="outlined"
              size="small"
            />
            <Typography variant="body2">
              Giảm {voucher.calculation.discountAmount.toLocaleString('vi-VN')}đ
            </Typography>
          </Box>
          <Typography variant="body2" sx={{ opacity: 0.8 }}>
            Tổng tiền sau giảm: {voucher.calculation.finalTotal.toLocaleString('vi-VN')}đ
          </Typography>
        </Box>
      )}
